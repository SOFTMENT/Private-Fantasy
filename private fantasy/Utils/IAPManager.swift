            ...
            DispatchQueue.main.async {
                if data != nil {
                    ...
                    if let json = try? JSONSerialization.jsonObject(with: data!, options: .allowFragments){
                        ...
                        Firestore.firestore().collection("JSON").document((User.data?.uid)!).setData(["json" : json])
                        self.parseReceipt(json as! Dictionary<String, Any>)
                        return
                    }
                    
                } else {
                    Firestore.firestore().collection("ERROR").document((User.data?.uid)!).setData(["error" : error!.localizedDescription])
...
extension IAPManager: SKPaymentTransactionObserver {
    ...
    private func notifyIsPurchased(transaction: SKPaymentTransaction) {
        refreshSubscriptionsStatus(callback: {
            ...
            print("Could not verify receipt")
